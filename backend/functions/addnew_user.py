...
def new_user(user_schema):
    new_user = Users(c_id = user_schema.c_id,fullname = user_schema.fullname,email = user_schema.email,password = passhash.hash(user_schema.password),contact_no = user_schema.contact_no, working_under = user_schema.working_under, dob = user_schema.dob, isactive = 1 , role_id = user_schema.role_id,created_at = datetime.datetime.now())
    db.add(new_user)
    db.commit()
    db.close()
